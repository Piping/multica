    ...
    entries: [
      {
        version: "0.3.5",
        date: "2026-05-21",
        title: "云端运行环境、按本地时区查看报表与更清晰的 Issue 状态",
        changes: [],
        features: [
          "开启后，可以直接在运行环境页面创建云端运行环境，并在同一处查看最近的节点状态",
          "报表会使用用户保存的查看时区，仪表盘和运行环境用量会按每个人预期的自然日展示",
        ...
        improvements: [
          ...
          "创建云端运行环境的表单更短，启动凭证由 Multica 自动处理",
        ...
        fixes: [
          ...
          "用量报表升级时，如果历史数据还没准备好，会先停止而不是发布空报表",
        ],
      },
      {
        version: "0.3.4",
