...
跟 `github_repo` 相比的取舍是有意的：只有绑定的那台守护进程能领同一个项目下的任务，而且 **同一个目录上的任务串行执行**，不再并行。换来的是：你的现有 checkout、当前分支、未提交的脏改动一切照旧 —— Multica 不会重新 clone。
...
如果"重新 clone"是最贵的步骤，选 `local_directory`；如果你需要同一仓库上多条任务并行，继续用 `github_repo`。
...
当本机的守护进程离线，或者这个项目已经在本机绑定了一个 `local_directory` 时，按钮会自动隐藏并给出说明。要把另一台机器上的目录绑过来，需要在那台机器装上桌面端、并在那边添加资源。
