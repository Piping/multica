...
Pick `local_directory` when re-cloning is the expensive part. Stay on `github_repo` when you want multiple agents working in parallel.
...
The button is also hidden (with a hint) when the daemon on this machine is offline, or when the project already has a `local_directory` bound to this daemon. To bind another machine's directory, install Desktop on that machine and add the resource from there.
